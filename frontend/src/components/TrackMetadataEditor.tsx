...
function PasteModal({ prompt, onApply, onClose }: PasteModalProps) {
  ...
  // Helpers: extract JSON object from mixed paste (JSON + markdown images) and markdown image URLs
  ...
  // Derive artwork preview URLs from markdown images + legacy JSON fields (for reference)
  ...
  function handleApply() {
    setError('')
    let raw = extractJsonSlice(json)
    // strip markdown hyperlinks inside string values: "[text](url)" → "url"
    raw = raw.replace(/"\[([^\]]*)\]\(([^)]*)\)"/g, '"$2"')
    try {
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 flex flex-col gap-4 p-5 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <button
            onClick={handleCopy}
            className="self-start text-xs px-3 py-1.5 rounded-lg bg-zinc-800 text-white hover:bg-zinc-700 transition-colors"
          >
        ...
        <div className="flex flex-col gap-1.5">
          <p className="text-xs font-medium text-zinc-500 uppercase tracking-wide">
            2 · Paste the JSON + images response here
          </p>
          ...
          {/* Artwork preview from pasted markdown images (optional, for reference) */}
          {previewArtworks.length > 0 && !previewParseError && (
            <div className="flex flex-col gap-2 rounded-lg border border-violet-200 bg-violet-50 p-3">
              <p className="text-xs font-medium text-violet-700">
                Detected artwork from paste — click to open / right-click Save, then upload via Cover art
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {previewArtworks.map((url, i) => (
                  <a
                    ...
                    className="group flex flex-col gap-1 rounded-lg border border-zinc-200 bg-white p-1.5 hover:border-violet-300 transition-colors"
                    title={url}
                  >
                    <img
                      src={url}
                      alt={`artwork option ${i + 1}`}
                      className="w-full aspect-square object-cover rounded bg-zinc-100"
                      loading="lazy"
                      onError={(e) => ((e.target as HTMLImageElement).style.display = 'none')}
                    />
                    <span className="text-[10px] leading-tight text-violet-600 group-hover:text-violet-700 break-all line-clamp-2">
                      Option {i + 1} ↗
                    </span>
